...
def create_dict_contigs(binning_folder: str, quant_file: str) -> list:
    """ Creates dictionary containing the proportions of the bins based on their TPM value

    :param binning_folder: Folder containing the bins
    :param quant_file: File containing the TPM data
    :return: List containing TPM information
    """
    bin_full = read_bin_file(binning_folder)
    quant_data = get_tpmfromfile(quant_file)
    quant_data["TPM_log2"] = np.log2(quant_data["TPM"] + 1)

    contig_dict = (calculate_realtive([mean(list(quant_data.loc[quant_data["Name"].isin(contig.keys())]["TPM_log2"])) for contig in bin_full]))
    contig_dict.sort()
    return contig_dict
...
# CHANGE THE VALUES BELOW TO YOUR OWN FOLDERS!
full_dict["samplefull"] = create_dict_contigs("samplefull/binning_output", "samplefull/quant.sf")
#full_dict["sample 1/2"] = create_dict_contigs("sample2/binning_output", "sample2/quant.sf")
#full_dict["sample 1/4"] = create_dict_contigs("sample4/binning_output", "sample4/quant.sf")

contig_dict = full_dict.values()

max_values = max([len(i) for i in full_dict.values()])


for i in full_dict.keys():
    while len(full_dict[i]) < max_values:
